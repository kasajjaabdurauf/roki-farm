...
- [x] v3.5.0: **auto-heal + one-tap updates** — `refreshFromRemote` now adopts the cloud's exact list whenever the outbox is empty (kills ghost records on every device automatically, no clearing site data, offline work still safe because pulls are skipped while pending); manual Resync stays; `/api/version` endpoint + 5-min/foreground/reconnect polling + "Update now" banner (one tap, auto-reloads unless unsynced); SW nudges open clients (ROKI_UPDATE) and listens for SKIP_WAITING; offline mode intentionally kept (rural/refugee-settlement patchy signal — removing it would risk losing registrations); 129 checks
- [x] v3.5.1: **agents get Data Check + instant refresh** — Data Check available to FIELD_AGENT (nav + dashboard "Validate data" button, previously admin-only; resync touches only the device's own local copy); agent banner shows live "Refreshes automatically every 15 seconds" + Data check + Sync now buttons; refresh-on-return (visibility/focus) added; everyone converges to the same cloud count via 15s auto-refresh + auto-heal; 135 checks
