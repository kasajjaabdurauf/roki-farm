...
import { ArrowLeft, ArrowRight, Check, CheckCircle2, Info, MapPin, Plus, Trash2, UserRound } from "lucide-react";
...
export function FarmerForm({ existing, onDone, selfRegistration }: { existing?: Farmer; onDone?: () => void; selfRegistration?: boolean }) {
  ...
  const [agentName, setAgentName] = useState(() => existing?.loggedBy || getAgentName());
  ...
  function stepErrors(i: number): string[] {
    const errs: string[] = [];
    if (i === 0) {
      // Only NEW registrations require the agent's name here; when editing
      // an existing farmer the existing credit is preserved automatically.
      if (!selfRegistration && !existing && !validText(agentName, 150)) errs.push("Your name (agent) is required so we can credit you");
  ...
  async function submit() {
    ...
    const staffName = selfRegistration ? "" : normalizeAgentName(agentName);
    ...
    const input = {
      ...
      survey,
      loggedBy: selfRegistration ? undefined : (staffName || existing?.loggedBy || undefined),
    };
  ...
  return (
    <div className="space-y-6">
      ...
      {step === 0 && (
        <div className="space-y-6">
          {/* AGENT CREDIT — unmissable, mandatory. This is the ONE place
              the agent's name is captured; it is stamped on the farmer
              record (loggedBy) and shown in Agent performance. */}
          {!selfRegistration && (
            <div className="rounded-2xl border-2 border-ochre-400 bg-ochre-50 p-4">
              <p className="flex items-center gap-2 text-[13.5px] font-bold tracking-wide text-ochre-900 uppercase">
                <UserRound className="h-4.5 w-4.5 shrink-0" /> Who is registering this farmer?
              </p>
              <p className="mt-1 text-[12.5px] leading-snug text-stone-600">
                Your name is stamped on this farmer and appears in{" "}
                <b>Agent performance</b>. This cannot be skipped — if there is no specific
                agent, write <b>none</b>.
              </p>
              <div className="mt-2.5">
                <Input
                  ...
                  placeholder="e.g. John Okello — or write 'none'"
                  className="h-12 text-[15px] font-semibold"
                  invalid={tried && !validText(agentName, 150)}
                />
                {tried && !validText(agentName, 150) && (
                  <p className="mt-1.5 flex items-center gap-1.5 text-[12.5px] font-semibold text-danger-600">
                    <Info className="h-3.5 w-3.5 shrink-0" /> Your name (agent) is required — type it or write “none”.
                  </p>
                )}
              </div>
              <p className="mt-1.5 text-[11.5px] text-stone-400">
                Remembered on this phone, so it's pre-filled for your next farmer.
              </p>
            </div>
          )}
          <SectionNote>
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <Field label="Sub-county" required hint="1.6 · type to search">
              <Input
                list="roki-subcounties"
                value={d.subCounty}
                onChange={(e) => set("subCounty", e.target.value)}
                placeholder={d.district ? "Type or pick a sub-county…" : "Pick district first"}
                disabled={!d.district}
              />
              <datalist id="roki-subcounties">
                {districtSubs.map((x) => (
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-stone-200 p-4">
            ...
            <div className="space-y-2 text-[13.5px]">
              {!selfRegistration && (
                <ReviewRow label="Agent" value={normalizeAgentName(agentName) || "N/A"} />
              )}
