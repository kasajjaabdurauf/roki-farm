...
import { addFarmer, updateFarmer } from "@/lib/db";
...
// The wizard
// ------------------------------------------------------------------
export function FarmerForm({ existing, onDone }: { existing?: Farmer; onDone?: () => void }) {
          ...
          <div>
            ...
            <div className="space-y-2">
              {d.previousCrops.map((p, idx) => (
                <div key={idx} className="grid grid-cols-[1fr_110px_110px_40px] items-center gap-2">
                  ...
                  <Input type="number" min={0} step="0.1" value={p.avgAreaAcres ?? ""} onChange={(e) => set("previousCrops", d.previousCrops.map((x, i) => (i === idx ? { ...x, avgAreaAcres: e.target.value ? parseFloat(e.target.value) : undefined } : x)))} placeholder="Acres" inputMode="decimal" />
                  <button
          ...
          <div>
            ...
            <div className="space-y-2.5">
              {d.currentCrops.map((c, idx) => (
                <div key={idx} className="grid grid-cols-2 items-center gap-2 rounded-xl border border-stone-200 bg-white p-3 sm:grid-cols-[1fr_100px_150px_120px_36px]">
                  ...
                  <Input type="number" min={0} step="0.1" value={c.areaAcres || ""} onChange={(e) => set("currentCrops", d.currentCrops.map((x, i) => (i === idx ? { ...x, areaAcres: parseFloat(e.target.value) || 0 } : x)))} placeholder="Acres" inputMode="decimal" />
        ...
        <div className="space-y-5">
          ...
          {d.wantsToSupplyRoki && (
            <div>
              ...
              <div className="space-y-2.5">
                {d.productions.map((p) => (
                  <div key={p.id} className="grid grid-cols-2 items-center gap-2 rounded-xl border border-stone-200 bg-white p-3 sm:grid-cols-[1fr_90px_120px_120px_36px]">
                    ...
                    <Input type="number" min={0} step="0.1" value={p.acres || ""} onChange={(e) => updateProduction(p.id, { acres: parseFloat(e.target.value) || 0 })} placeholder="Acres" inputMode="decimal" />
                    ...
                    <div className="flex items-center gap-1">
                      <Input type="number" min={0} value={p.expectedVolumeKg || ""} onChange={(e) => updateProduction(p.id, { expectedVolumeKg: parseFloat(e.target.value) || 0 })} placeholder="kg" inputMode="numeric" />
                      <button
                        onClick={() => set("productions", d.productions.filter((x) => x.id !== p.id))}
                        className="grid h-11 w-9 shrink-0 place-items-center rounded-lg text-stone-400 hover:bg-danger-50 hover:text-danger-600"
                        aria-label="Remove row"
                      >
                        <Trash2 className="h-4 w-4" />
                      </button>
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-2xl border border-stone-200 p-4">
            ...
            <div className="space-y-2 text-[13.5px]">
              <ReviewRow label="Farmer" value={d.fullName || "—"} />
              <ReviewRow label="Phone" value={d.phone ? `${d.phone} · ${GENDER_LABEL[d.gender]}` : "—"} />
              <ReviewRow label="Location" value={[d.parish, d.village, d.subCounty, d.district].filter(Boolean).join(", ") || "—"} />
              ...
              <ReviewRow label="Experience" value={`${FARMING_YEARS_OPTIONS.find((o) => o.value === d.farmingYears)?.label ?? ""} · ${d.farmingTypes.join(", ") || "—"}`} />
              <ReviewRow label="Current crops" value={d.currentCrops.map((c) => c.crop).join(", ") || "—"} />
              <ReviewRow label="Roki production plan" value={d.productions.length === 0 ? "—" : `${d.productions.length} crops · ${totalPlanAcres.toFixed(1)} ac · ≈ ${(totalPlanKg / 1000).toFixed(1)} t`} />
            </div>
